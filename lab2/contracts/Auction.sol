...
contract Auction {
    ...
    function finalize() public {
        require(outcome == Outcome.SUCCESSFUL, "Auction isn't successful!");
        require(
            judgeAddress == address(0) ||
            msg.sender == judgeAddress ||
            msg.sender == highestBidderAddress, "Sender can't call finalize()!");

        payUp(payable(sellerAddress), address(this).balance);
    }
    ...
    function refund() public {
        require(outcome == Outcome.NOT_SUCCESSFUL, "Auction should be unsuccessful!");
        require(
            judgeAddress == address(0) ||
            msg.sender == sellerAddress ||
            msg.sender == judgeAddress, "Sender can't call refund()!");
        require(highestBidderAddress != address(0), "Not one bid has been placed for an item!");

        payUp(payable(highestBidderAddress), address(this).balance);
    }

    function payUp(address payable _to, uint _amount) internal {
